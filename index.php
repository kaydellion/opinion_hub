...
// Get categories with poll count (exclude suspended polls for non-admins)
$user_role = isLoggedIn() ? getCurrentUser()['role'] : null;
$status_filter = ($user_role === 'admin') ? "p.status IN ('active', 'paused')" : "p.status = 'active'";

$categories = $conn->query("SELECT c.*, COUNT(p.id) as poll_count
                            FROM categories c
                            LEFT JOIN polls p ON c.id = p.category_id AND {$status_filter}
                            GROUP BY c.id
                            HAVING poll_count > 0
...
if (isLoggedIn()) {
    ...
    $following_check = $conn->query("SELECT COUNT(*) as following_count FROM user_follows WHERE follower_id = $user_id");
    $is_following_someone = $following_check && $following_check->fetch_assoc()['following_count'] > 0;

    if ($is_following_someone) {
        // Get polls from followed creators
        $personalized_polls = $conn->query("SELECT DISTINCT p.*, c.name as category_name,
                                          (SELECT COUNT(*) FROM poll_responses WHERE poll_id = p.id) as total_responses,
                                          u.first_name, u.last_name
                                          FROM polls p
                                          LEFT JOIN categories c ON p.category_id = c.id
                                          JOIN users u ON p.created_by = u.id
                                          JOIN user_follows uf ON p.created_by = uf.following_id
                                          WHERE {$status_filter}
...
<div class="hero-section"
    style="background: linear-gradient(135deg, #FF6B35 0%, #004E89 100%); color: white; padding: 100px 0; position: relative; overflow: hidden;">
    ...
    <div
        style="position: absolute; bottom: 0; left: 0; right: 0; height: 100px; background: rgba(255,255,255,0.1); transform: skewY(-2deg);">
    </div>
</div>
        ...
        <div class="col-md-4 mb-4">
            <div class="card border-0 shadow-sm h-100">
                <div class="card-body p-4">
                    ...
                    <h2 class="display-4 fw-bold text-primary"><?php echo number_format($stats['total_users'] ?? 0); ?>
                    </h2>
        ...
        <div class="col-md-4 mb-4">
            <div class="card border-0 shadow-sm h-100">
                <div class="card-body p-4">
                    ...
                    <h2 class="display-4 fw-bold text-primary"><?php echo number_format($stats['total_polls'] ?? 0); ?>
                    </h2>
            ...
            <div class="card border-0 shadow-sm h-100">
                <div class="card-body p-4">
                    ...
                    <h2 class="display-4 fw-bold text-primary">
                        <?php echo number_format($stats['total_responses'] ?? 0); ?>
                    </h2>
                    ...
                    <a href="<?php echo SITE_URL; ?>polls.php?category=<?php echo $cat['id']; ?>" class="text-decoration-none">
                        <div class="card border-0 shadow-sm h-100 hover-scale">
                            <div class="card-body text-center p-4">
                                ...
                                <p class="text-muted mb-0"><?php echo $cat['poll_count']; ?>
                                    <?php echo $cat['poll_count'] == 1 ? 'poll' : 'polls'; ?>
                                </p>
                            </div>
                        </div>
                    </a>
            ...
            <div class="row">
                <div class="col-lg-9">
                    <div id="pollsCarousel" class="carousel slide" data-bs-ride="carousel">
                                                ...
                                                <div class="card h-100 border-0 shadow-sm hover-scale">
                                                    <?php if (!empty($poll['image'])): ?>
                                                        <img src="<?php echo SITE_URL . 'uploads/polls/' . $poll['image']; ?>"
                                                            class="card-img-top" alt="Poll image"
                        ...
                        <button class="carousel-control-prev" type="button" data-bs-target="#pollsCarousel"
                            data-bs-slide="prev" style="width: 5%;">
                            <span class="carousel-control-prev-icon bg-dark rounded-circle p-3" aria-hidden="true"></span>
                        </button>
                        <button class="carousel-control-next" type="button" data-bs-target="#pollsCarousel"
        ...
        <div class="container">
            ...
            <!-- <div class="text-center mt-4">
                <a href="<?php echo SITE_URL; ?>polls.php?filter=following" class="btn btn-success btn-lg px-4">
                    <i class="fas fa-star"></i> View All Followed Polls
                </a>
            </div> -->
        </div>
    </div>
