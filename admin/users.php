...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    if ($action === 'activate') {
        ...
        if (!$update) {
            error_log('User activate update prepare failed: ' . $conn->error);
            $_SESSION['error'] = 'Database error while activating user.';
        } else {
        ...
        if (!$update) {
            error_log('User suspend update prepare failed: ' . $conn->error);
            $_SESSION['error'] = 'Database error while suspending user.';
        } else {
            $update->bind_param("i", $target_user_id);
            if ($update->execute()) {
                createNotification(
        ...
        $update = $conn->prepare("UPDATE users SET status = 'deleted' WHERE id = ?");
        if (!$update) {
            error_log('User delete update prepare failed: ' . $conn->error);
            $_SESSION['error'] = 'Database error while deleting user.';
        } else {
            $update->bind_param("i", $target_user_id);
            if ($update->execute()) {
    ...
    header("Location: users.php" . (isset($_GET['status']) ? '?status=' . $_GET['status'] : ''));
    exit();
}
...
$query = "SELECT u.*,
          ...
          (SELECT COUNT(*) FROM poll_responses WHERE respondent_id = u.id) as total_responses,
          (SELECT COUNT(*) FROM blog_posts WHERE user_id = u.id) as total_blog_posts,
          (SELECT COUNT(*) FROM blog_comments WHERE user_id = u.id) as total_comments
          FROM users u
          WHERE $where_clause
        ...
        <div class="col-12">
            ...
            <div class="row mb-4">
                ...
                $stats = [
                    ...
                    'active' => $conn->query("SELECT COUNT(*) as count FROM users WHERE role = 'user' AND status = 'active'")->fetch_assoc()['count'],
                    'suspended' => $conn->query("SELECT COUNT(*) as count FROM users WHERE role = 'user' AND status = 'suspended'")->fetch_assoc()['count'],
                ];
                ?>
                <div class="col-md-4">
                ...
                <div class="col-md-4">
                    <div class="card text-center">
                        <div class="card-body">
                            <h3 class="text-warning"><?php echo number_format($stats['suspended']); ?></h3>
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                ...
                                                <?php if ($u['phone']): ?>
                                                    ...
                                                    <small><?php echo htmlspecialchars($u['phone']); ?></small>
                                                <?php endif; ?>
                                            </td>
                                            <td>
                                                <?php
                                                ...
                                                $badge_color = $status_badges[$u['status']] ?? 'secondary';
                                                ?>
                                            ...
                                            <td>
                                                <small>
                                                    ...
                                                    <?php echo $u['total_responses']; ?> responses<br>
                                                    <i class="fas fa-blog text-success me-1"></i>
                                                    <?php echo $u['total_blog_posts']; ?> posts<br>
                                                    <i class="fas fa-comment text-info me-1"></i>
                                                    <?php echo $u['total_comments']; ?> comments
                                                </small>
                                            </td>
                                            <td>
                                                ...
                                                <div class="btn-group btn-group-sm">
                                                    <?php if ($u['status'] === 'active'): ?>
                                                        <button class="btn btn-warning text-white" 
                                                                onclick="showSuspendModal(<?php echo $u['id']; ?>, '<?php echo htmlspecialchars($u['full_name']); ?>')">
                                                    ...
                                                    <?php if ($u['status'] !== 'deleted'): ?>
                                                        <button class="btn btn-danger text-white" 
                                                                onclick="confirmDelete(<?php echo $u['id']; ?>, '<?php echo htmlspecialchars($u['full_name']); ?>')">
        ...
        <div class="modal-content">
            <form method="POST">
                ...
                <div class="modal-body">
                    ...
                    <p class="text-muted">The user account will be permanently marked as deleted.</p>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancel</button>
